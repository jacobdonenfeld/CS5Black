...
def make_alien(starting_position, starting_vel=vector(0, 0, 0)):
    ...
    alien_eye1 = sphere(size=0.3 * vector(1, 1, 1), pos=.42 * vector(.7, .5, .2), color=color.red)
    alien_eye2 = sphere(size=0.3 * vector(1, 1, 1), pos=.42 * vector(.2, .5, .7), color=color.red)
    alien_eye3 = sphere(size=0.3 * vector(1, 1, 1), pos=.42 * vector(.3, 1, .4), color=color.red)
    alien_hat = cylinder(pos=0.42 * vector(0, .9, -.2), axis=vector(.02, .2, -.02), size=vector(0.2, 0.3, 0.7),
                         color=color.yellow)
...
wallC = box(pos=vector(0, 0, 10), axis=vector(1, 0, 0), size=vector(20, 1, .2), color=vector(1.0, 0.5, 0.5))  # amber
wallD = box(pos=vector(10, 0, 0), axis=vector(0, 0, 1), size=vector(20, 1, .2), color=color.red)  # blue
...
def run_away(runner, away):
    amt = 0.02

    if away.pos.z > runner.pos.z:  # hit - check for z
        runner.vel = runner.vel + vector(0, 0, -amt)
    else:
        runner.vel = runner.vel + vector(0, 0, amt)
    if away.pos.x > runner.pos.x:  # hit - check for x
        runner.vel = runner.vel + vector(-amt, 0, 0)
    else:
        runner.vel = runner.vel + vector(amt, 0, 0)
