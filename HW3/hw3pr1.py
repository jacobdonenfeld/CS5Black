...
def scale(L, scale_factor):
    map(lambda x: x*scale_factor, L)


# Function to write #2:  add_2

def add_2(L, M):
        return list(map(lambda x: L[x] + M[x], range(min(len(L), len(M)))))
...
def add_3(L, M, P):
    return list(map(lambda x: L[x] + M[x] + P[x], range(min(len(L), len(M), len(P)))))


# Function to write #4:  add_scale_2
def add_scale_2(L, M, L_scale, M_scale):
    return list(map(lambda x: L[x]*L_scale + M[x]*M_scale, range(min(len(L), len(M)))))
...
def replace_some(L, chance_of_replacing):
    return list(map(lambda x: float(randomize(x, chance_of_replacing)), L))
...
def echo(filename1, time_delay):
    ...
    samps2 = [0]*int(sr1*time_delay) + samps1
    sr2 = sound_data2[1]
    print("Computing new sound...")
    # this gets the midpoint and calls it x
    newsamps = add_scale_2(samps1, samps2, 0.5, 0.5)
...
def add_scale_3(L, M, N, L_scale, M_scale, N_scale):
    return list(map(lambda x: L[x]*L_scale + M[x]*M_scale + N[x]*N_scale, range(min(len(L), len(M), len(N)))))
